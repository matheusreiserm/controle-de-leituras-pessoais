...
const rawCsv = fs.readFileSync('raw_input.csv', 'utf8');
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (foundMonth) {
    if (foundMonth === 'Janeiro' && parsedBooks.length > 0) {
      const last = parsedBooks[parsedBooks.length - 1];
  ...
  if (parts.length >= 6) {
    ...
    let format = 'Físico';
    if (globalIdCounter % 3 === 0) format = 'Kindle';
    if (globalIdCounter % 11 === 0) format = 'PDF';
    if (globalIdCounter % 17 === 0) format = 'Archive.org';

    let language = 'Português';
    const isSpanishTitle = /[áéíóúñ]/i.test(title) || ['Aira', 'Bellatin', 'Levrero', 'Cucurto', 'Sosa Villada'].some(a => author.includes(a));
    const isEnglishTitle = /^[A-Za-z0-9\s\,\.\'\:\-\?]+$/.test(title) && !/[áéíóúâêîôûãõçà]/i.test(title);

    if (isSpanishTitle) {
      language = 'Espanhol';
    } else if (isEnglishTitle && (nationality === 'Estados Unidos' || nationality === 'Reino Unido' || nationality === 'Austrália' || nationality === 'Canadá')) {
      language = 'Inglês';
    } else if (globalIdCounter % 4 === 0 && nationality !== 'Brasil') {
      language = 'Inglês';
    }
    ...
    parsedBooks.push({
      id: globalIdCounter++,
      readingYear: years[blockIndex] || 2025,
      yearBookId: rawId || 1,
...
console.log('Total parsed books:', parsedBooks.length);
console.log('2023 count:', parsedBooks.filter(b => b.readingYear === 2023).length);
console.log('2024 count:', parsedBooks.filter(b => b.readingYear === 2024).length);
console.log('2025 count:', parsedBooks.filter(b => b.readingYear === 2025).length);

const tsContent = `import { Book } from '../types';\n\nexport const INITIAL_BOOKS: Book[] = ` + JSON.stringify(parsedBooks, null, 2) + `;\n`;

fs.writeFileSync('src/data/initialBooks.ts', tsContent);
console.log('Successfully written initialBooks.ts!');
